...
export const AnimMenu = () => {
    ...
    tlMenu
        .fromTo(
            ".btn_menu_open",
            { y: "0" },
            { y: "-100%", duration: 0.2, delay: 0 }
        )
        .to(navWrapper, { y: "0", duration: 0.2, delay: 0, ease: "none" })
        .fromTo(
            ".btn_menu_close",
            { y: "-100%" },
            { y: "0", duration: 0.1, delay: 0 }
        )
        .to(".nav_list_item_link", { y: 0, delay: 0, stagger: 0.2 });
    ...
    function navigateToSection(){
        resetMenu();
        gsap.timeline({ ease: "power4.inOut" })
